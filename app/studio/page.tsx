...
export default async function StudioPage() {
    ...
    // We can also fetch the automations to show status.
    const automations = await (prisma as any).automation.findMany({ where: { isActive: true } });
    ...
    // Mock data if empty to show the concept
            ...
            <main className="pt-32 pb-12 max-w-7xl mx-auto px-4 relative z-10">
                <header className="mb-12 border-b border-white/10 pb-6">
                    ...
                    <p className="text-gray-400 font-mono mt-2">
                        Monitoring 4 active neural agents generating multi-platform content.
                    </p>
                    {hasActiveWorkflow ? (
                        <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-green-500/10 text-green-500 border border-green-500/20 rounded font-mono text-xs">
                            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                            N8N UPLINK ESTABLISHED
                        </div>
                    ) : (
                        <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 rounded font-mono text-xs">
                            <AlertCircle size={12} />
                            N8N UPLINK DISCONNECTED
                        </div>
                    )}
                </header>
